  ...
  if (size === 'icon') {
    return (
      <div className={`flex items-center justify-center ${className} h-8 w-8 rounded-md bg-sidebar-accent`}>
         <span className={`text-xl font-extrabold text-sidebar-primary-foreground`}>7</span>
      </div>
    );
  }
    ...
    <div className={`flex items-center space-x-2 ${className}`}>
      <h1 className={`${titleTextSizeClass} font-bold ${textColorClass} whitespace-nowrap`}>
        <span className="font-extrabold">
          <span className="text-accent">7</span><span className="text-destructive">-</span><span className="text-primary">11</span>
        </span>
