...
// Note: This component is now dynamically imported by its parent page.

export function AIInsightsContent() {
  const { user, role } = useAuth();
  const { toast } = useToast();
  const [devicesForScope, setDevicesForScope] = useState<Device[]>([]);
  const [currentScopeName, setCurrentScopeName] = useState<string>('nation-wide');
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      if (role === 'branch' && user.branchId) {
        const branchDevices = await getDevicesByBranchIdSnapshot(user.branchId);
        ...
        if (role === 'regional' && user.regionId) {
          ...
          for (const b of branchesInUserRegion) {
              const branchDevices = await getDevicesByBranchIdSnapshot(b.id);
          ...
          const allDevicesData = await getAllDevicesSnapshot();
  ...
  const handleScopeChange = useCallback(async () => {
    ...
    try {
        if (selectedBranchId) {
            newDevices = await getDevicesByBranchIdSnapshot(selectedBranchId);
            ...
            newDevices = await getAllDevicesSnapshot();
        }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 mb-6 bg-muted/30 border rounded-lg">
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => {
                    setSelectedRegionId(value === "all" ? undefined : value);
